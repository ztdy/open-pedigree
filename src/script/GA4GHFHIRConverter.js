...
GA4GHFHIRConverter.exportAsFHIR = function (pedigree, privacySetting, knownFhirPatienReference, pedigreeImage) {
  ...
  // Use the marked proband (falls back to node 0). Node 0 stays the layout anchor, but the
  // clinical proband may have been moved to another individual.
  let probandId = 0;
  let ggProps = pedigree.GG.properties || {};
  for (let pid in ggProps) {
    if (ggProps.hasOwnProperty(pid) && ggProps[pid] && ggProps[pid].proband && pedigree.GG.isPerson(pid)) {
      probandId = parseInt(pid, 10);
      break;
    }
  }
  let probandRef = this.processTreeNode(probandId, pedigree, privacySetting, knownFhirPatienReference, pedigreeIndividuals,
