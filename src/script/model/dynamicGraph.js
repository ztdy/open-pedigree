...
DynamicPositionedGraph.prototype = {
  ...
  // The clinical proband: the person carrying the proband marker (`proband:true` in its
  // properties), falling back to node 0 (the internal layout anchor) when none is marked.
  // Person groups never qualify. This is the authoritative proband for clinical/relatedness
  // and export purposes; node 0 stays the layout/ranking anchor regardless.
  getProbandId: function() {
    var props = this.DG.GG.properties;
    for (var id in props) {
      if (props.hasOwnProperty(id) && props[id] && props[id].proband) {
        var nid = parseInt(id, 10);
        if (this.isPerson(nid) && !this.isPersonGroup(nid)) {
          return nid;
        }
      }
    }
