...
var NodeMenu = Class.create({
  initialize : function(data, tabs, otherCSSClass) {
    ...
    this.form.select('input.suggest-genes').each(function(item) {
      if (!item.hasClassName('initialized')) {
        // Desktop build: resolve gene symbols against the bundled offline HGNC dataset
        // over opdata:// instead of the (unreachable) PhenoTips GeneNameService.
        var geneDesktop = !!(window.openPedigreeDesktop && window.openPedigreeDesktop.isDesktop);
        var geneServiceURL = new XWiki.Document('GeneNameService', 'PhenoTips').getURL('get', 'outputSyntax=plain');
        item._suggest = new PhenoTips.widgets.Suggest(item, {
          script: geneDesktop ? 'opdata://genes/?json=true&' : (geneServiceURL + '&json=true&'),
          varname: 'q',
          noresults: 'No matching terms',
    ...
    this.form.select('input.suggest-hpo').each(function(item) {
      if (!item.hasClassName('initialized')) {
        // Desktop build: resolve HPO terms against the bundled offline HPO dataset over
        // opdata:// and bypass the Solr query processor (it targets a live Solr backend).
        var hpoDesktop = !!(window.openPedigreeDesktop && window.openPedigreeDesktop.isDesktop);
        var solrServiceURL = HPOTerm.getServiceURL();
        item._suggest = new PhenoTips.widgets.Suggest(item, {
          script: hpoDesktop ? 'opdata://hpo/?' : (solrServiceURL + 'rows=100&'),
          queryProcessor: hpoDesktop ? null : (typeof(PhenoTips.widgets.SolrQueryProcessor) == 'undefined' ? null : new PhenoTips.widgets.SolrQueryProcessor({
