...
var ImportSelector = Class.create( {
  ...
  initialize: function() {
    ...
    var promptImport = new Element('div', {'class': 'import-section'}).update(I18n.t('Import data:'));
    ...
    if (!!window.FileReader && !!window.FileList) {
      ...
      var uploadLink = new Element('div', {'class': 'import-upload'}).update('(<a>' + I18n.t('Select a local file to be imported') + '</a>)');
    ...
    configListElement.insert(_addConfigOption(true,  I18n.t('Treat non-standard phenotype values as new disorders'), 'accept'));
    configListElement.insert(_addConfigOption(false, I18n.t('Treat non-standard phenotype values as "no information"'), 'dontaccept'));

    var markEvaluated = new Element('input', {'type' : 'checkbox', 'value': '1', 'name': 'mark-evaluated'});
    var markLabel1     = new Element('label', {'class': 'import-mark-label1'}).insert(markEvaluated).insert(I18n.t('Mark all patients with known disorder status with \'documented evaluation\' mark')).wrap('td').wrap('tr');
    ...
    var markLabel2   = new Element('label', {'class': 'import-mark-label2'}).insert(markExternal).insert(I18n.t('Save individual IDs as given in the input data as \'external ID\'')).wrap('td').wrap('tr');
    ...
    buttons.insert(new Element('input', {type: 'button', name : 'import', 'value': I18n.t('Import'), 'class' : 'button', 'id': 'import_button'}).wrap('span', {'class' : 'buttonwrapper'}));
    ...
    this.dialog = new PhenoTips.widgets.ModalPopup(mainDiv, {close: {method : this.hide.bind(this), keys : closeShortcut}}, {extraClassName: 'pedigree-import-chooser', title: I18n.t('Pedigree import'), displayCloseButton: true});
  },
  ...
  _onImportStarted: function() {
    ...
    if (!importValue || importValue == '') {
      alert(I18n.t('Nothing to import!'));
      return;
    }

    var importType = $$('input:checked[type=radio][name="select-type"]')[0].value;
    // (import type not logged)
