...
var PersonHoverbox = Class.create(AbstractHoverbox, {
  ...
  generateButtons: function($super) {
    ...
    // The proband can't be removed; neither can node 0, which stays the internal layout
    // anchor even when the proband marker has been moved to another individual.
    if (!this.getNode().isProband() && this.getNode().getID() != 0) {
