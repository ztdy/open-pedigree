...
var Controller = Class.create({
  ...
  handleRemove: function(event) {
    ...
    var removeSelected = function() {
      try {
        ...
        // A removal can take the marked proband with it (e.g. deleting a connector node that
        // disconnects a descendant proband from node 0). Re-normalize so the marker falls
        // back to node 0 and the arrow is refreshed rather than left on a deleted node.
        editor.getView().ensureSingleProband();
  ...
  handleSetProperty: function(event) {
    ...
    // The proband marker is a single, exclusive, cross-node change ("move the marker from A
    // to B"). The per-node undo event below cannot express that, so handle it atomically with
    // a full-state undo/redo snapshot. Keeps node 0 as the layout anchor; only a real Person
    // (never a partnership/relationship/person-group) can carry the marker.
    if (properties && properties.hasOwnProperty('setProband')) {
      Controller._handleSetProband(nodeID, properties.setProband, event.memo.noUndoRedo);
      return;
    }
...
// Move the exclusive proband marker to `nodeID`. Clears every other person's marker (view +
// model), sets the new one, and records a full-state undo so undo/redo restore exactly one
// proband. Unchecking is ignored (the UI disables it) to avoid a zero-proband state.
Controller._handleSetProband = function(nodeID, value, noUndoRedo) {
  var graph = editor.getGraph();
  var view  = editor.getView();
  var node  = view.getNode(nodeID);
  var isGroup = graph.isPersonGroup && graph.isPersonGroup(nodeID);
  if (!node || typeof node.setProband !== 'function' || isGroup) {
    return; // only a concrete Person can be the proband
  }
  if (!value) {
  ...
  for (var otherID in nodeMap) {
    if (!nodeMap.hasOwnProperty(otherID)) {
      continue;
    }
    var other = nodeMap[otherID];
    if (other && other !== node && typeof other.getProband === 'function' && other.getProband()) {
      other.setProband(false);
      graph.setProperties(other.getID(), other.getProperties());
    }
  }
  node.setProband(true);
  graph.setProperties(nodeID, node.getProperties());
  editor.getNodeMenu().update();
  if (!noUndoRedo) {
    // Full-state snapshot (no per-node event): undo restores the prior graph, redo this one.
    editor.getActionStack().addState(null, null);
  }
};
