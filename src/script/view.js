...
var View = Class.create({
  ...
  /**
     * Enforces exactly one proband after a full (re)build of the graph. The intended
     * proband is whichever person node has an explicit `proband:true` in the serialized
     * model (a marker the user moved); if none does, node 0 is the proband by default.
     * Every other person is cleared. Call after loading/importing a graph.
     *
     * @method ensureSingleProband
     */
  ensureSingleProband: function() {
    var graph = editor.getGraph();
    // Authoritative: the marked person, else node 0; never a partnership/relationship/group.
    var probandID = graph.getProbandId();
    ...
    for (var id in nodeMap) {
      ...
      var n = nodeMap[id];
      if (!n || typeof n.setProband !== 'function' || (graph.isPersonGroup && graph.isPersonGroup(n.getID()))) {
        continue; // concrete Person nodes only
      }
      var isTheProband = (n.getID() === probandID);
      n.setProband(isTheProband);
      // Normalize the model so duplicate/stale `proband` markers (hand-edited or imported
      // graphs) can't resurface on the next reload and silently override the user's choice.
      var props = graph.getProperties(n.getID());
      if (props) {
        if (isTheProband && !props.proband) {
          props.proband = true;
          graph.setProperties(n.getID(), props);
        } else if (!isTheProband && props.proband) {
          delete props.proband;
          graph.setProperties(n.getID(), props);
        }
      }
    }
