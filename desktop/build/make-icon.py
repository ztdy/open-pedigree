...
# macOS .icns. Apple's icon grid puts the tile body at ~824/1024 with transparent margin
# (full-bleed art looks oversized next to other Dock icons), so inset before saving.
icns_path = os.path.join(out_dir, "icon.icns")
body = int(S * 824 / 1024)
mac = Image.new("RGBA", (S, S), (0, 0, 0, 0))
mac.paste(img.resize((body, body), Image.LANCZOS), ((S - body) // 2, (S - body) // 2))
mac.save(icns_path)
